...
\begin{enumerate}
\item \textbf{[Ref 1]} Prozan (date, source)
\item \textbf{[Ref 2]} Cuffel, J.W., et al. (date, source)
\item \textbf{[Ref 3]} Wehofer, W.E. and Moger, W.T. (date, source)
\item \textbf{[Ref 4]} Laval, H. (date, source)
\item \textbf{[Ref 5]} Migdal, D., et al. (date, source)
\item \textbf{[Ref 6]} Brown, C.A. and Ozcan, H. (date, source)
\item \textbf{[Ref 7]} Serra, R.A. (date, source)
\item \textbf{[Ref 8]} (date, source)
\item \textbf{[Ref 9]} (date, source)
\item \textbf{[Ref 10]} MacCormack, R.W. (date, source)
\item \textbf{[Ref 11]} von Neumann, J. and Richtmyer, R.D. (date, source)
\item \textbf{[Ref 12]} Moretti, G. and Abbett, M. (date, source)
\item \textbf{[Ref 13]} Cline, M.C. (date, source)
\item \textbf{[Ref 14]} (date, source)
\item \textbf{[Ref 15]} (date, source)
\item \textbf{[Ref 16]} Prozan, R.H. and Kooker, D.E. (date, source)
\item \textbf{[Ref 17]} Thornock, T.P. (date, source)
\item \textbf{[Ref 18]} Bresnahan, D.L. and Johns, A.L. (date, source)
\end{enumerate}
...
\section{Main Program (MAIN.F)}

\begin{lstlisting}[language=Fortran,basicstyle=\tiny\ttfamily]
[Main program Fortran code to be included from nap/fortran code/fortran_main.f]
[Due to file size, full listing will be formatted in final build]
\end{lstlisting}

\section{Geometry Subroutine (GEOM.F)}

\textbf{[Fortran code to be inserted: geom.f - Located in nap/fortran code/geom.f]}

\section{Inlet Subroutine (INLET.F)}

\textbf{[Fortran code to be inserted: inlet.f]}

\section{Wall Subroutine (WALL.F)}

\textbf{[Fortran code to be inserted: wall.f]}

\section{Interior Mesh Subroutine (INTER.F)}

\textbf{[Fortran code to be inserted: inter.f]}

\section{Mass Flow Subroutine (MASFLO.F)}

\textbf{[Fortran code to be inserted: masflo.f]}

\section{One-Dimensional Initialization (ONEDIM.F)}

\textbf{[Fortran code to be inserted: onedim.f]}

\section{MacCormack Scheme (SHOCK.F)}

\textbf{[Fortran code to be inserted: shock handling routine]}
