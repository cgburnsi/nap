...
10	TCONV=0.0 $ FDT=1.0 $ TSTOP=1.0 $ NASM=1 $ NSTAG=0 $ NAME=0
	...
	N1D=1 $ NDIM=1 $ THETA(1)=0.0 $ PE=14.7 $ NST=0 $ N=0 $ IEX=1
	NCONVI=1 $ IERR=0 $ JFLAG=0 $ IUI=1 $ IUO=1 $ SMP=0.95 $ ISUPER=0
...
40 	PRINT 660
	...
	PRINT 730, LMAX,MMAX,NMAX,NPRIND,TCONV,FDT,NSTAG,NASM,IUNIT,IUI,IUO,IEX,NCONVI,TSTOP,N1D,NPLOT,IPUNCH,ISUPER,IAV,CAV,XMU,XLA,RKMU,CTA,LSS,SMP,NST
	PRINT 670
...
60	XICB=XI
	...
	Y0=0.0
	DO 80 L=1,LMAX
	IF (NDIM,EQ,0) Y=YW(L)-YCB(L)
	IF (NDIM,EQ,1) Y=YW(L)**2-YCB(L)**2
...
100	CONTINUE
	PRINT 670
	IF (IUI,EQ,1) PRINT 760, PT(1),TT(1),THETA(1),PE
	IF (IUI,EQ,2) PRINT 770, PT(1),TT(1),THETA(1),PE
	GO TO 130
110	PRINT 660
...
280 NLINE=NLINE+1
	...
	PRINT 670
	NLINE=1
290 PRINT 850, L,M,XP,YP,UP,VP,PRES,RHO,VELMAG,XMACH,TEMP
300 CONTINUE
...
410 	DO 420 M=1,MMAX
		...
		RO(LMAX,M,N3)=RO(L1,M,N3)+IEX*(RO(L1,M,N3)-RO(L2,M,N3))
		IF (P(LMAX,M,N3),GT,0.0,AND,RO(LMAX,M,N3),GT,0.0) GO TO 420
		P(LMAX,M,N3)=P(L1,M,N3)
		RO(LMAX,M,N3)=RO(L1,M,N3)
420 	CONTINUE
...
C
		...
		DO 430 L=LDUM,LMAX
		DO 430 M=1,MMAX
		IF (U(L,M,N1),EQ,0.0) GO TO 430
		DQ=ABS((U(L,M,N3)-U(L,M,N1))/U(L,M,N1))
		IF (DQ,GT,DQM) DQM=DQ
430 	CONTINUE
440 	NC=NC+1
		NPC=NPC+1
...
480 	DO 540 IU=1,2
		...
		DO 510 M=1,MMAX
		Y=Y+DYIO
		VELMAG=SQRT(U(L,M,N3)**2+V(L,M,N3)**2)
...
490 	NLINE=NLINE+1
		...
		PRINT 670
		NLINE=1
500 	PRINT 850, L,M,XP,YP,UP,VP,PRES,RHO,VELMAG,XMACH,TEMP
510 	CONTINUE
		IF (IU,EQ,2) GO TO 520
		PRINT 870, MASST,THRUST,MASSI,MASSE
		GO TO 530
520 	MASST=MASST*0.4536
		MASSI=MASSI*0.4536
		MASSE=MASSE*0.4536
		THRUST=THRUST*4.4477
		PRINT 880, MASST,THRUST,MASSI,MASSE
