...
class StructuredGrid:
    def __init__(self):
        self.lmax = 21          # [-] Number of grid points in x-axis
        self.mmax = 8           # [-] Number of grid points in y-axis
        self.xi   = 0.31
        self.ri   = 2.50
        self.rt   = 0.80
        self.xe   = 4.05
...
def build_nap_grid(
    ...
    """Build NAP-like data structures for geometry + flow storage."""
    ani = np.deg2rad(ani_deg)
    ane = np.deg2rad(ane_deg)
    xt, re, lt, yw, nxny = geometry(lmax, xi, ri, rt, xe, rci, rct, ani, ane)
    xw = np.linspace(xi, xe, lmax)

    # Start with no centerbody; these arrays match NAP naming.
    xcb = xw.copy()
    ycb = np.zeros(lmax)
    nxnycb = np.zeros(lmax)

    x, y = _build_structured_coordinates(xw, yw, ycb, mmax, ndim)
    boundaries = _build_boundary_index_sets(lmax, mmax, lt)
    jac, xi_x, xi_y, eta_x, eta_y = _compute_mapping_metrics(x, y)

    # NAP stores 2 time levels (N=1,2); keep same shape in Python.
    shape_3d = (lmax, mmax, 2)
    u = np.zeros(shape_3d, dtype=float, order="F")
    v = np.zeros(shape_3d, dtype=float, order="F")
    p = np.zeros(shape_3d, dtype=float, order="F")
    ro = np.zeros(shape_3d, dtype=float, order="F")

    # Boundary condition arrays across M.
    pt = np.zeros(mmax)
    tt = np.zeros(mmax)
    theta = np.zeros(mmax)
